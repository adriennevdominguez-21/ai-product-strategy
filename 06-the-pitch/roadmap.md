...
3. The economics: At the expected usage level, the model produces $60 in monthly revenue per user against $19.25 in COGS, increasing projected gross margin from 64% to 68% and gross profit by $11.75 per user per month. Those economics depend on unvalidated usage and willingness-to-pay assumptions, so the first three district pilots must test pricing and 10x-usage sensitivity before we scale.
